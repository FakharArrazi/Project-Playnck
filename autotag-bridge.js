...
const USER_AGENT = `Playnck/${app.getVersion()} ( https://github.com/FakharArrazi/Project-Playnck )`;
...
const JUNK_BRACKET_RE =
  /[(\[][^)\]]*\b(official|video|audio|lyrics?|visualizer|mv|hd|hq|4k|explicit)\b[^)\]]*[)\]]/gi;
...
async function autoTagTrack(filePath, hint, mode) {
  if (!filePath)
    return { found: false, reason: "This track's file location isn't known." };

  mode = mode === "fingerprint" || mode === "text" ? mode : "auto";

  hint = hint || {};
  const hintArtist =
    hint.artist && !PLACEHOLDER_ARTIST.test(hint.artist.trim())
      ? hint.artist
      : "";
      ...
      try {
        ...
        if (fp.ok) {
          ...
          if (!matches || !matches.length)
            fingerprintNote =
              "audio fingerprinting ran, but AcoustID had no match for it";
        } else {
  ...
  if (!matches || !matches.length) {
    ...
    if (mode === "fingerprint") {
      ...
      reason =
        "No confident match found. (Fingerprinting is off — no AcoustID API key is configured, so only a title/artist search was tried.)";
  ...
  for (const m of matches) {
    m.images = await gatherCoverArtCandidates(
      m.releases,
      IMAGES_PER_MATCH,
      MAX_RELEASES_TRIED_PER_MATCH,
    );
    delete m.releases;
...
function resolveFpcalcPath() {
  ...
  if (app.isPackaged) {
    candidates.push(
      path.join(process.resourcesPath, "fpcalc", process.platform, bin),
    );
  } else {
    candidates.push(
      path.join(__dirname, "resources", "fpcalc", process.platform, bin),
    );
  }
  ...
  return new Promise((resolve) => {
    ...
    execFile(bin, ["-json", filePath], { timeout: 20000 }, (err, stdout) => {
      if (err) {
        ...
        console.error(
          "autoTagTrack: fpcalc unavailable or failed:",
          err.message,
        );
      ...
      try {
        ...
        if (!parsed.fingerprint || !parsed.duration) {
          return resolve({
            ok: false,
            error:
              "fpcalc ran but didn't return a usable fingerprint for this file",
          });
        }
        resolve({
          ok: true,
...
async function acoustidLookup(fingerprint, duration) {
  const url =
    `${ACOUSTID_LOOKUP_URL}?client=${encodeURIComponent(ACOUSTID_CLIENT_KEY)}` +
  ...
  const seen = new Set();
  const candidates = [];
  idLoop: for (const result of sorted) {
  ...
  for (const c of candidates) {
    ...
    matches.push(
      buildMatchFromRecording(mbRecording || c.fallback, "fingerprint"),
    );
  }
...
async function musicbrainzTextSearch(title, artist) {
  ...
  const loose = await musicbrainzQuery(
...
async function musicbrainzQuery(query) {
  ...
  if (confident.length) {
    built.sort(
      (a, b) => b.releaseScore - a.releaseScore || b.mbScore - a.mbScore,
    );
  }
...
function buildMatchFromRecording(recording, source) {
  ...
  const sorted = [...releases].sort(
    (a, b) => scoreRelease(b) - scoreRelease(a),
  );
  ...
  return {
    ...
    releases: sorted
      .map((r) => ({
        ...
        title:
          r.title || (r["release-group"] && r["release-group"].title) || null,
        date: r.date || null,
      }))
      .filter((r) => r.id),
  };
...
function scoreRelease(release) {
  ...
  const secondaryTypes = Array.isArray(rg["secondary-types"])
    ? rg["secondary-types"]
    : [];
  let score = 0;
...
async function fetchReleaseGroupCoverArt(releaseGroupId) {
  ...
  const res = await fetch(
    `${COVER_ART_ARCHIVE}/release-group/${releaseGroupId}/front-500`,
    {
      headers: { "User-Agent": USER_AGENT },
    },
  );
