(function () {
  ...
  function wireMediaKeys() {
    ...
    const artImg = document.querySelector(
      '.art-slot[data-role="current"] .art-slot-img',
    );
    ...
    function updateMetadata() {
      ...
      const hasArt =
        artImg && !artImg.classList.contains("hidden") && artImg.src;
      navigator.mediaSession.metadata = new MediaMetadata({
        title: title,
        artist: artist,
        artwork: hasArt
          ? [{ src: artImg.src, sizes: "512x512", type: "image/png" }]
          : [],
      });
    }

    updateMetadata();
    new MutationObserver(updateMetadata).observe(titleEl, {
      childList: true,
      characterData: true,
      subtree: true,
    });
    if (artistEl)
      new MutationObserver(updateMetadata).observe(artistEl, {
        childList: true,
        characterData: true,
        subtree: true,
      });
    if (artImg)
      new MutationObserver(updateMetadata).observe(artImg, {
        attributes: true,
        ...
        window.electronAPI
          ...
          .then((result) => {
            if (!result || !result.trashed) {
              console.warn(
                "Couldn't move file to Recycle Bin:",
                p,
