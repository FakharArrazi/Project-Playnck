...
async function exportLibraryBackup() {
  ...
  const [tracks, playlists, folders, playlistFolders, lyrics, settingsRows] =
    await Promise.all([
      idbGetAll("tracks"),
      idbGetAll("playlists"),
      idbGetAll("folders"),
  ...
  if (result && result.saved)
    return { saved: true, filePath: result.filePath, skippedNoPath };
...
async function importLibraryBackup() {
  ...
  const picked = await window.electronAPI.openTextFile("Playnck Backup", [
    "json",
  ]);
  ...
  let payload;
  try {
    payload = JSON.parse(picked.content);
  } catch {
  ...
  for (const pf of payload.playlistFolders || [])
    await idbPut("playlistFolders", pf);
...
async function onBackupImportClick() {
  ...
  if (result.imported) {
    statusEl.textContent = tr("backup.imported", {
...
function buildLanguageBodyHTML() {
  const chips = Object.keys(LANGUAGES)
    .map(
      (code) =>
...
const APP_VERSION_FALLBACK = "1.2.2";
