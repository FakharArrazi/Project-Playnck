...
async function createPlaylistFolderPrompt() {
  const name = await promptModal(
    tr("prompt.newPlaylistFolderTitle"),
...
function openPlaylistFolderMenu(e, folder) {
  ...
  moveBtn.addEventListener("click", () => {
    closeMenu();
    openMoveItemModal(folder, "folder");
  });
...
function deletePlaylistFolder(folder) {
  ...
  const playlistsInside = state.playlists.filter(
    (p) => p.parentId && ids.has(p.parentId),
  );

  const parts = [];
  if (playlistsInside.length)
    parts.push(plural(playlistsInside.length, "playlist"));
  if (descendantFolderCount)
    parts.push(plural(descendantFolderCount, "folder"));
  ...
  if (
    ...
    deletedPlaylistIds.has(state.filter.playlistId)
  ) {
    state.filter = null;
  }
...
function countPlaylistsInFolder(folderId) {
  ...
  return state.playlists.filter((p) => p.parentId && ids.has(p.parentId))
    .length;
}
