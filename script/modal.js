...
function wireModalDismissal(cancelValue, resolve) {
  let settled = false;
  ...
  function onOutsideCancel() {
    finish(cancelValue);
  }
  function onOverlayClick(e) {
    if (e.target.id === "modalOverlay") finish(cancelValue);
  }
...
function promptModal(title, label, defaultValue) {
  return new Promise((resolve) => {
    ...
    const finish = wireModalDismissal(null, resolve);
...
function updateAvailableModal(version) {
  return new Promise((resolve) => {
    ...
    const finish = wireModalDismissal(false, resolve);
