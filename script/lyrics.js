...
function syncLyrics(cur) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].time <= adjustedCur) activeIdx = i;
    else break;
  }
...
async function openSyncModal(track) {
  ...
  if (!t) {
    openModal(
      tr("side.syncLyrics"),
      `<p class='info-empty'>${escapeHTML(tr("empty.nothingPlayingSync"))}</p>`,
    );
    return;
  }
  ...
  openModal(
    tr("side.syncLyrics"),
    `<p class='info-empty'>${escapeHTML(tr("lyrics.loading"))}</p>`,
  );
  ...
  if (!lines || !lines.length) {
    openModal(
      tr("side.syncLyrics"),
      `<p class='info-empty'>${escapeHTML(tr("lyrics.notFound"))}</p>`,
    );
    return;
  }
  if (lines[0].time === null) {
    openModal(
      tr("side.syncLyrics"),
      `<p class='info-empty'>${escapeHTML(tr("lyrics.notTimeSynced"))}</p>`,
    );
    return;
  }
  ...
  const renderOffset = () => {
    inputEl.value = (offsetMs > 0 ? "+" : "") + offsetMs;
  };
  renderOffset();
  ...
  $("modalBody")
    .querySelectorAll(".sync-nudge-btn")
    .forEach((btn) => {
