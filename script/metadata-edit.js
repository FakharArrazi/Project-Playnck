...
function openEditModal(track) {
  ...
  function renderMatchOptions(matches) {
    ...
    $("editAutoTagMatchSelect").addEventListener("change", (e) =>
      applyMatch(parseInt(e.target.value, 10) || 0),
    );
  }
  ...
  async function runAutoTag(mode) {
    if (!(
      window.electronAPI &&
      window.electronAPI.autoTagTrack &&
      t.filePath
    )) {
    ...
    const cleanArtistHint = /^unknown artist$/i.test(artistHint)
      ? ""
      : artistHint;
    ...
    if (!result || !result.found) {
      autoTagStatus.textContent = tr("edit.autoTagNotFound", {
  ...
  autoTagFingerprintBtn.addEventListener("click", () =>
    runAutoTag("fingerprint"),
  );
  ...
  $("editSaveBtn").addEventListener("click", async () => {
    ...
    if (leftoverActions)
      leftoverActions.closest(".edit-status-actions").remove();
    ...
    if (!(result && result.written)) {
      ...
      if (status) {
        ...
        status.textContent = tr("edit.fileWriteFailed", {
    ...
    if (window.electronAPI.renameFile) {
      ...
      if (renameResult && renameResult.renamed && renameResult.newPath) {
        ...
        renameFailedReason =
          (renameResult && renameResult.reason) ||
          tr("edit.couldntRenameGeneric");
      }
    }
    ...
    if (status) {
      ...
      if (result.imageIgnored) {
        ...
        status.textContent = tr("edit.savedTagsButNotRenamed", {
          reason: renameFailedReason,
        });
      }
    }
    setTimeout(closeModal, 1400);
