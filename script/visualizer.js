...
function saveVisualizerSettings() {
  idbPut("settings", {
    ...
    value: {
      ...
      intensity: state.visualizer.intensity,
    },
  }).catch(() => {});
}
...
function updateVisualizerState() {
  ...
  const shouldRun =
    state.visualizer.enabled && !audioEl.paused && !!state.currentTrack;
...
function drawVisualizerFrame() {
  ...
  if (
    !visualizerFreqData ||
    visualizerFreqData.length !== analyserNode.frequencyBinCount
  ) {
  ...
  for (let i = 0; i < barCount; i++) {
    ...
    const alpha = Math.max(
      0,
      Math.min(1, (0.12 + v * 0.22) * state.visualizer.intensity),
    );
