...
function formatBitrate(bytes, seconds) {
  if (!bytes || !seconds || !isFinite(seconds) || seconds <= 0)
    return "Unknown";
...
function formatTechMeta(meta, filePath) {
  ...
  if (
    codecLabel &&
    containerLabel &&
    codecLabel.toUpperCase() !== containerLabel.toUpperCase()
  ) {
...
function hideWithMotion(element, duration = 180) {
  ...
  setTimeout(() => {
    if (element.classList.contains("motion-out"))
      element.classList.add("hidden");
  }, duration);
}
