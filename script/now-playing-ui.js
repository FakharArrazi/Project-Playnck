...
import { replayMotion, fallbackArt, formatTechMeta } from "./utils.js";
import { peekNextEntry, peekPrevEntry } from "./queue.js";
import { getTrackArtURL } from "./init.js";
import { navSwipeDir, setNavSwipeDir } from "./player.js";
...
function slotWithRole(role) {
  return carouselSlots.find((s) => s.role === role);
}
...
function recycleSlotInstant(slot, role, entry) {
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      slot.el.classList.remove("art-slot-instant");
    });
  });
}
...
function syncCarouselStatic() {
  ...
  const currentEntry = state.currentTrack
  ...
  current.el.dataset.role = "current";
  prev.role = "prev";
  prev.el.dataset.role = "prev";
  next.role = "next";
  next.el.dataset.role = "next";
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      [current, prev, next].forEach((s) =>
        s.el.classList.remove("art-slot-instant"),
      );
    });
  });
}
...
function refreshNextPreview() {
  initCarouselSlots();
  const prev = slotWithRole("prev"),
    next = slotWithRole("next");
  [prev, next].forEach((s) => {
    if (!s) return;
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      [prev, next].forEach((s) => {
        if (s) s.el.classList.remove("art-slot-instant");
      });
    });
  });
}
...
function rotateCarousel(dir) {
  initCarouselSlots();
  if (!slotWithRole("current")) {
    syncCarouselStatic();
    return;
  }

  if (dir === "next") {
    const oldPrev = slotWithRole("prev"),
      oldCurrent = slotWithRole("current"),
      oldNext = slotWithRole("next");
    oldCurrent.role = "prev";
    oldCurrent.el.dataset.role = "prev";
    oldNext.role = "current";
    oldNext.el.dataset.role = "current";
    recycleSlotInstant(oldPrev, "next", peekNextEntry());
  } else {
    const oldPrev = slotWithRole("prev"),
      oldCurrent = slotWithRole("current"),
      oldNext = slotWithRole("next");
    oldCurrent.role = "next";
    oldCurrent.el.dataset.role = "next";
    oldPrev.role = "current";
    oldPrev.el.dataset.role = "current";
...
function roundedQuad(pts, r) {
  ...
  for (let i = 0; i < n; i++) {
    const prev = pts[(i - 1 + n) % n],
      curr = pts[i],
      next = pts[(i + 1) % n];
    const lenPrev = edgeLen(prev, curr),
      lenNext = edgeLen(curr, next);
    const rIn = lenPrev < 1e-6 ? 0 : Math.min(r, lenPrev / 2);
    const rOut = lenNext < 1e-6 ? 0 : Math.min(r, lenNext / 2);
    const dirPrev =
      lenPrev < 1e-6
        ? [0, 0]
        : [(curr[0] - prev[0]) / lenPrev, (curr[1] - prev[1]) / lenPrev];
    const dirNext =
      lenNext < 1e-6
        ? [0, 0]
        : [(next[0] - curr[0]) / lenNext, (next[1] - curr[1]) / lenNext];
...
function buildD(quadA, quadB, r) {
  return roundedQuad(quadA, r) + " " + roundedQuad(quadB, r);
}
