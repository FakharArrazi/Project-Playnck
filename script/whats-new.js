...
function primaryGroupHTML(entry){
  const dateHTML=entry.date ? `<p class="theme-note whats-new-date">${escapeHTML(entry.date)}</p>` : "";
...
function whatsNewBodyHTML(primaryEntry){
  if(!primaryEntry) return `<p class="info-empty">${escapeHTML(tr("whatsNew.empty"))}</p>`;
  const older=CHANGELOG.filter(entry=>entry.version!==primaryEntry.version).slice(0,MAX_OLDER_GROUPS);
...
function openWhatsNewModal(entry){
  const title=entry ? tr("whatsNew.titlePrefix",{version:"v"+entry.version}) : tr("whatsNew.btn");
  openModal(title, whatsNewBodyHTML(entry));
}


function showWhatsNewManually(){
  openWhatsNewModal(findChangelogEntry(state.appVersion) || latestChangelogEntry());
}
