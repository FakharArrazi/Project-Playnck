...
async function handleMissingTrack(track) {
  ...
  let existence = {};
  try {
    existence = await window.electronAPI.checkPathsExist(checkPaths);
  } catch (e) {
    console.warn("handleMissingTrack: checkPathsExist failed", e);
    return;
  }
...
audioEl.addEventListener("timeupdate", () => {
  const cur = audioEl.currentTime,
    dur =
      audioEl.duration ||
      (state.currentTrack && state.currentTrack.duration) ||
      0;
...
audioEl.addEventListener("pause", () => {
  if (playProgress) playProgress.lastTs = null;
});
...
audioEl.addEventListener("ended", () => {
  if (crossfadeState) completeCrossfadeHandoff();
  else nextTrack(true);
});
audioEl.addEventListener("timeupdate", maybeStartCrossfade);
audioEl.addEventListener("pause", () => {
  if (fadeAudioEl && crossfadeState) fadeAudioEl.pause();
});
audioEl.addEventListener("play", () => {
  if (fadeAudioEl && crossfadeState && fadeAudioEl.paused)
    fadeAudioEl.play().catch(() => {});
});
