...
function openSideDropdown() {
  ...
  setTimeout(
    () => document.addEventListener("click", closeSideDropdown, { once: true }),
    0,
  );
}
...
function openInfoModal(track) {
  ...
  if (!t) {
    openModal(
      tr("info.modalTitleEmpty"),
      `<p class='info-empty'>${escapeHTML(tr("empty.nothingPlayingInfo"))}</p>`,
    );
    return;
  }
  ...
  const estimatedBitrate = file
    ? formatBitrate(file.size, t.duration)
    : tr("common.unknown");
  const fileNameFallback = t.filePath
  ...
  const rows = [
    ...
    [
      tr("info.rowFileType"),
      file && file.type ? file.type : tr("common.unknown"),
      "infoFileTypeVal",
    ],
    [
      tr("info.rowFileSize"),
      file ? formatBytes(file.size) : tr("common.unknown"),
      "infoFileSizeVal",
    ],
    [tr("info.rowBitrate"), estimatedBitrate, "infoBitrateVal"],
    [
      tr("info.rowDateAdded"),
      t.dateAdded
