...
function resolveNextIndex() {
  ...
  if (state.shuffle) {
    if (state.queue.length > 1) {
      ...
      if (shuffleNextPick && shuffleNextPick.forId === forId)
        return shuffleNextPick.index;
      let r;
      do {
        r = Math.floor(Math.random() * state.queue.length);
      } while (r === state.queueIndex);
...
export { resolveNextIndex, peekNextEntry, peekPrevEntry, resetShuffleNextPick };
