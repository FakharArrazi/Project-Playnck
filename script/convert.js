...
function renderConvertTab() {
  ...
  if (!window.electronAPI) {
    wrap.appendChild(
      el("div", "empty-state", escapeHTML(tr("convert.desktopOnly"))),
    );
    return;
  }
...
function renderConvertFFmpegSetup(wrap, mode) {
  ...
  if (mode === "installing") {
    ...
    state.convert.installLog.forEach((line) =>
      log.appendChild(el("div", "", escapeHTML(line))),
    );
  ...
  if (mode === "install-failed") {
    ...
    if (state.convert.installError) {
      section.appendChild(
        el(
          "div",
          "convert-install-error",
          escapeHTML(state.convert.installError),
        ),
      );
    }
    const retryBtn = el(
      "button",
      "amr-add-btn",
      escapeHTML(tr("convert.tryAgain")),
    );
  ...
  section.appendChild(
    el("p", "", escapeHTML(tr("convert.ffmpegRequiredNote"))),
  );
  const installBtn = el(
    "button",
    "edit-save-btn",
    escapeHTML(tr("convert.installFFmpeg")),
  );
...
function renderConvertReadyBanner(wrap) {
  ...
  if (state.convert.ffmpegVersion)
    left.appendChild(
      el("span", "", escapeHTML("v" + state.convert.ffmpegVersion)),
    );
...
function renderConvertAddFilesSection(wrap) {
  ...
  section.appendChild(
    el("div", "theme-group-label", escapeHTML(tr("convert.addFiles"))),
  );
  ...
  const browseBtn = el(
    "button",
    "amr-add-btn",
    escapeHTML(tr("convert.browseFiles")),
  );
  ...
  const addFolderBtn = el(
    "button",
    "amr-add-btn",
    `<svg viewBox='0 0 24 24' width='14' height='14' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z'/></svg> ${escapeHTML(tr("convert.addFolder"))}`,
  );
...
async function pushConvertQueueItem(
  queue,
  filePath,
  displayName,
  fallbackSize,
) {
  if (queue.some((q) => q.path === filePath)) return false;
  let meta = null;
  try {
  ...
  queue.push({
    ...
    ext,
    sizeBytes:
      meta && meta.fileSize != null ? meta.fileSize : fallbackSize || null,
...
async function addFilesToConvertQueue(fileList) {
  ...
  for (const file of files) {
    ...
    if (!filePath) {
      console.warn(
        "Convert: couldn't resolve a real path for",
        file.name,
        "— skipping.",
      );
      continue;
    ...
    if (await pushConvertQueueItem(c.queue, filePath, displayName, file.size))
      added++;
  }
...
async function addFolderToConvertQueue() {
  ...
  if (!paths || !paths.length) {
    openModal(
      tr("convert.addFolder"),
      `<p class="info-empty">${escapeHTML(tr("convert.noNewFiles"))}</p>`,
    );
    return;
  }
  ...
  for (const filePath of paths) {
    const base = filePath
      ...
      .pop()
      .replace(/\.[^.]+$/, "");
    if (await pushConvertQueueItem(c.queue, filePath, base, null)) added++;
  }
  if (added && state.currentTab === "convert") renderTab();
}

const CONVERT_QUEUE_ICON =
  "<svg viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M9 18V5l12-2v13'/><circle cx='6' cy='18' r='3'/><circle cx='18' cy='16' r='3'/></svg>";
const CONVERT_REMOVE_ICON =
...
function renderConvertQueueSection(wrap) {
  ...
  header.appendChild(
    el("div", "theme-group-label", escapeHTML(tr("convert.queueTitle"))),
  );
  if (c.queue.length && !c.isConverting) {
    const clearBtn = el(
      "button",
      "edit-cancel-btn",
      escapeHTML(tr("convert.clearQueue")),
    );
  ...
  if (!c.queue.length) {
    section.appendChild(
      el("div", "empty-state", escapeHTML(tr("convert.queueEmpty"))),
    );
    wrap.appendChild(section);
    return;
  }
...
function buildConvertQueueRow(item) {
  ...
  if (item.status === "failed" && item.error) {
    info.appendChild(
      el("div", "convert-queue-row-error", escapeHTML(item.error)),
    );
  }
  ...
  const status = el(
    ...
    escapeHTML(tr("convert.status." + item.status)),
  );
...
function renderConvertSettingsSection(wrap) {
  ...
  const section = el("div", "convert-section");
  section.appendChild(
    el("div", "theme-group-label", escapeHTML(tr("convert.settingsTitle"))),
  );

  const grid = el("div", "convert-settings-grid");

  const formatBlock = el("div", "");
  formatBlock.appendChild(
    el("div", "convert-field-label", escapeHTML(tr("convert.outputFormat"))),
  );
  ...
  if (info.lossless && c.format !== "flac" && c.format !== "wav") {
    grid.appendChild(
      el("p", "convert-lossless-note", escapeHTML(tr("convert.losslessNote"))),
    );
    ...
    block.appendChild(
      el(
        "div",
        "convert-field-label",
        escapeHTML(tr("convert.flacCompression")),
      ),
    );
    ...
    for (let level = 0; level <= 8; level++) {
      const chip = el(
        "button",
        "lang-chip" +
          (c.settings.flac.compressionLevel === level ? " active" : ""),
        String(level),
      );
      chip.type = "button";
      chip.addEventListener("click", () => {
        c.settings.flac.compressionLevel = level;
        renderTab();
      });
    ...
    block.appendChild(
      el(
        "p",
        "convert-lossless-note",
        escapeHTML(tr("convert.flacCompressionNote")),
      ),
    );
    ...
    const block = el("div", "");
    block.appendChild(
      el("div", "convert-field-label", escapeHTML(tr("convert.bitDepth"))),
    );
    const row = el("div", "convert-chip-row");
    [16, 24].forEach((depth) => {
      const chip = el(
        "button",
        "lang-chip" + (c.settings.wav.bitDepth === depth ? " active" : ""),
        depth + "-bit",
      );
      chip.type = "button";
      chip.addEventListener("click", () => {
        c.settings.wav.bitDepth = depth;
        renderTab();
      });
      row.appendChild(chip);
    });
    ...
    const block = el("div", "");
    block.appendChild(
      el("div", "convert-field-label", escapeHTML(tr("convert.bitrate"))),
    );
    const row = el("div", "convert-chip-row");
    CONVERT_BITRATES[c.format].forEach((kbps) => {
      const chip = el(
        "button",
        "lang-chip" +
          (c.settings[c.format].bitrateKbps === kbps ? " active" : ""),
        kbps + " kbps",
      );
      chip.type = "button";
      chip.addEventListener("click", () => {
        c.settings[c.format].bitrateKbps = kbps;
        renderTab();
      });
      row.appendChild(chip);
    });
...
function renderConvertOutputSection(wrap) {
  ...
  const section = el("div", "convert-section");
  section.appendChild(
    el("div", "theme-group-label", escapeHTML(tr("convert.outputTitle"))),
  );

  section.appendChild(
    el("div", "convert-field-label", escapeHTML(tr("convert.outputFolder"))),
  );
  const pathRow = el("div", "convert-output-path-row");
  pathRow.appendChild(
    el("div", "convert-output-path", escapeHTML(c.outputFolder || "")),
  );
  const chooseBtn = el(
    "button",
    "amr-add-btn",
    escapeHTML(tr("convert.chooseFolder")),
  );
  ...
  collisionBlock.appendChild(
    el("div", "convert-field-label", escapeHTML(tr("convert.ifFileExists"))),
  );
  const collisionRow = el("div", "convert-chip-row");
  [
    ["rename", "convert.collision.rename"],
    ["replace", "convert.collision.replace"],
    ["skip", "convert.collision.skip"],
  ].forEach(([mode, key]) => {
    const chip = el(
      "button",
      "lang-chip" + (c.collisionMode === mode ? " active" : ""),
      escapeHTML(tr(key)),
    );
    chip.type = "button";
    chip.addEventListener("click", () => {
      c.collisionMode = mode;
      renderTab();
    });
...
async function chooseConvertOutputFolder() {
  const dir = await window.electronAPI.selectFolder(
    state.convert.outputFolder || undefined,
  );
  if (!dir) return;
...
function renderConvertProgressSection(wrap) {
  ...
  currentLabelRow.appendChild(
    el(
      "span",
      "",
      escapeHTML(
        tr("convert.currentFile") +
          (current ? ": " + (current.title || current.name) : ""),
      ),
    ),
  );
  ...
  overallLabelRow.appendChild(
    el("span", "", escapeHTML(tr("convert.overallProgress"))),
  );
  overallLabelRow.appendChild(
    el(
      "span",
      "",
      escapeHTML(
        tr("convert.filesOf", { done: c.overallDone, total: c.overallTotal }),
      ),
    ),
  );
  ...
  const overallPct = c.overallTotal
    ? (c.overallDone / c.overallTotal) * 100
    : 0;
...
function renderConvertControlsRow(wrap) {
  ...
  if (c.isConverting) {
    ...
    const startBtn = el(
      "button",
      "edit-save-btn",
      escapeHTML(tr("convert.startConversion")),
    );
...
function renderConvertCompletionBanner(wrap) {
  ...
  if (summary.failed)
    summaryText += tr("convert.completeSummaryFailed", {
      count: summary.failed,
    });
  if (summary.skipped)
    summaryText += tr("convert.completeSummarySkipped", {
  ...
  section.appendChild(
    el(
      "div",
      "convert-complete-path",
      escapeHTML(tr("convert.outputLocation", { path: summary.outputFolder })),
    ),
  );
  ...
  const openBtn = el(
    "button",
    "amr-add-btn",
    escapeHTML(tr("convert.openOutputFolder")),
  );
  openBtn.addEventListener("click", () =>
    window.electronAPI.openFolder(summary.outputFolder),
  );
  actions.appendChild(openBtn);
  const dismissBtn = el(
    "button",
    "edit-cancel-btn",
    escapeHTML(tr("convert.startNewBatch")),
  );
  dismissBtn.addEventListener("click", () => {
    state.convert.lastRunSummary = null;
    renderTab();
  });
...
function handleConvertProgressTick({ jobId, percent }) {
  ...
  if (labelRow && labelRow.lastElementChild)
    labelRow.lastElementChild.textContent = Math.round(percent) + "%";

  const row = document.querySelector(
    '.convert-queue-row[data-job-row-id="' +
      CSS.escape(jobId) +
      '"] .convert-queue-row-progress-fill',
  );
