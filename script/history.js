...
function trackHistoryProgress() {
  ...
  if (!track || !historyProgress || historyProgress.trackId !== track.id)
    return;
...
audioEl.addEventListener("pause", () => {
  if (historyProgress) historyProgress.lastTs = null;
});
...
function historyRowHTML(entry) {
  ...
  const time = new Date(entry.playedAt).toLocaleTimeString([], {
    hour: "numeric",
    minute: "2-digit",
  });
...
function bindHistoryRowClicks() {
  $("modalBody")
    .querySelectorAll(".history-row[data-track-id]")
    .forEach((row) => {
...
function openHistoryModal() {
  ...
  if (!state.playHistory.length) {
    openModal(
      tr("history.title"),
      `<p class="info-empty">${escapeHTML(tr("empty.noListeningHistory"))}</p>`,
    );
    return;
  }
