...
import {
  createPlaylistFolderPrompt,
  openPlaylistFolder,
  openPlaylistFolderMenu,
  countPlaylistsInFolder,
} from "./playlist-folders.js";
...
function computeAlbums() {
  ...
  for (const t of libraryTracks()) {
    ...
    if (!map.get(key).art && getTrackArtURL(t))
      map.get(key).art = getTrackArtURL(t);
  }
...
function sortGroups(groups, nameField) {
  ...
  const totalDuration = (g) =>
    g.tracks.reduce((sum, t) => sum + (t.duration || 0), 0);
  const newestDate = (g) =>
    g.tracks.reduce((max, t) => Math.max(max, t.dateAdded || 0), 0);
  const oldestDate = (g) =>
    g.tracks.reduce(
      (min, t) => Math.min(min, t.dateAdded || Infinity),
      Infinity,
    );
  switch (state.sortBy) {
    case "title-desc":
    ...
    case "artist-desc":
      ...
      break;
    case "duration-asc":
      sorted.sort((a, b) => totalDuration(a) - totalDuration(b));
      break;
    case "duration-desc":
      sorted.sort((a, b) => totalDuration(b) - totalDuration(a));
      break;
    case "date-desc":
      sorted.sort((a, b) => newestDate(b) - newestDate(a));
      break;
    case "date-asc":
      sorted.sort((a, b) => oldestDate(a) - oldestDate(b));
      break;
    case "title-asc":
    default:
...
function sortTracks(tracks) {
  ...
  switch (sortBy) {
    ...
    case "date-asc":
      ...
      break;
    case "track-asc":
      sorted.sort(compareByTrackNumber);
      break;
    case "title-asc":
    default:
...
function renderTab() {
  ...
  const sortApplies =
    ...
    state.currentTab === "albums" ||
    state.currentTab === "artists";
  $("sortToggle").classList.toggle("hidden", !sortApplies);
  const searchApplies =
    state.filter ||
    (state.currentTab !== "home" && state.currentTab !== "convert");
  ...
  if (state.currentTab === "home") {
    ...
    if (
      ...
      !state.playlistFolders.find((f) => f.id === state.playlistFolderId)
    ) {
      state.playlistFolderId = null;
    }
    const currentFolder = state.playlistFolderId
      ? state.playlistFolders.find((f) => f.id === state.playlistFolderId)
      : null;
    listTitle.textContent = currentFolder
      ? currentFolder.name
      : tr("nav.playlists");
...
function renderSongList(
  ...
  if (virtualized) {
    ...
    if (!virtualScrollListenerBound) {
      listContainer.addEventListener("scroll", scheduleVirtualSongRender, {
        passive: true,
      });
  ...
  tracks.forEach((t) => {
    listContainer.appendChild(buildSongRow(t, playlistIdContext, queueTracks));
  });
  if (virtualized) {
...
function albumGroupOrderValue(group) {
  const mode = state.sortBy;
  if (mode === "duration-asc" || mode === "duration-desc")
    return group.tracks.reduce((sum, t) => sum + (t.duration || 0), 0);
  if (mode === "date-asc" || mode === "date-desc")
    return Math.max(...group.tracks.map((t) => t.dateAdded || 0));
  return null;
}

function compareAlbumGroups(a, b) {
  const mode = state.sortBy;
  if (mode === "duration-asc" || mode === "date-asc")
    return albumGroupOrderValue(a) - albumGroupOrderValue(b);
  if (mode === "duration-desc" || mode === "date-desc")
    return albumGroupOrderValue(b) - albumGroupOrderValue(a);
  if (mode === "title-desc") return b.album.localeCompare(a.album);
...
function renderArtistAlbumSections(tracks, scrollTarget = null) {
  ...
  tracks.forEach((t) => {
    const key = albumGroupKey(t);
    if (!groups.has(key)) {
      const group = { album: t.album, tracks: [] };
      groups.set(key, group);
      order.push(group);
    }
    groups.get(key).tracks.push(t);
  });
  order.forEach((group) => group.tracks.sort(compareByTrackNumber));
...
function refreshPlayingHighlight() {
  listContainer
    .querySelectorAll(".song-row.playing")
    .forEach((r) => r.classList.remove("playing"));
  const id = state.currentTrack && state.currentTrack.id;
  if (id == null) return;
  listContainer
    .querySelectorAll(`.song-row[data-track-id="${CSS.escape(String(id))}"]`)
...
function scrollToNowPlaying() {
  ...
  if (existingRow) {
    const target = Math.max(
      0,
      existingRow.offsetTop -
        listContainer.clientHeight / 2 +
        existingRow.offsetHeight / 2,
    );
...
function refreshSelectionHighlight(id) {
  ...
  listContainer
    .querySelectorAll(`[data-select-id="${CSS.escape(String(id))}"]`)
    .forEach((elm) => {
...
function updateSelectionBar() {
  ...
  $("selAddPlaylistBtn").classList.toggle(
    "hidden",
    state.selectType === "playlists",
  );
}
...
function currentSelectType() {
  ...
  if (
    !state.filter &&
    ["albums", "artists", "playlists", "folders"].includes(state.currentTab)
  )
...
function renderAlbumGrid(albums, scrollTarget = null) {
  ...
  albums.forEach((a) => {
    ...
    img.loading = "lazy";
    img.decoding = "async";
    img.src = a.art || fallbackArt();
    imgWrap.appendChild(img);
    if (state.selectMode)
      imgWrap.appendChild(el("div", "row-check card-check"));
...
function renderPlaylistList(scrollTarget = null) {
  ...
  const folders = state.playlistFolders.filter(
...
function renderPlaylistFolderRow(f) {
  ...
  wrap.appendChild(
    el("div", "sub", plural(countPlaylistsInFolder(f.id), "playlist")),
  );
  line.appendChild(wrap);
  const menuBtn = el("button", "menu-btn", "&#8942;");
  menuBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    openPlaylistFolderMenu(e, f);
  });
...
function renderFolderList() {
  ...
  const addSongsBtn = el(
    "button",
    "",
    `<svg viewBox='0 0 24 24' width='14' height='14' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round'><path d='M9 18V5l12-2v13'/><circle cx='6' cy='18' r='3'/><circle cx='18' cy='16' r='3'/></svg> ${escapeHTML(tr("folder.addSongs"))}`,
  );
  ...
  const addFolderBtn = el(
    "button",
    "",
    `<svg viewBox='0 0 24 24' width='14' height='14' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z'/></svg> ${escapeHTML(tr("folder.addFolder"))}`,
  );
  ...
  state.folders.forEach((f) => {
    ...
    line.addEventListener("click", () => {
      if (state.selectMode) toggleItemSelected(f.id);
        ...
        state.filter = { type: "folder", title: f.name, tracks };
        renderTab();
      }
    });
    listContainer.appendChild(line);
