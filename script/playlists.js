...
async function createPlaylistPrompt(trackIdToAdd) {
  const name = await promptModal(
    tr("prompt.newPlaylistTitle"),
    tr("prompt.playlistNameLabel"),
  );
  if (!name) return;
  const p = {
    id: uid(),
    ...
    parentId: state.playlistFolderId || null,
    order: nextOrder(),
  };
...
function openPlaylistMenu(e, playlist) {
  ...
  moveBtn.addEventListener("click", () => {
    closeMenu();
    openMoveItemModal(playlist, "playlist");
  });
  ...
  positionMenu(menu, e.currentTarget, { offset: 150 });
...
function deletePlaylist(playlist) {
  ...
  if (
    state.filter &&
    state.filter.type === "playlist" &&
    state.filter.playlistId === playlist.id
  ) {
    state.filter = null;
  }
  renderTab();
...
function openMoveItemModal(item, kind) {
  const excludedIds =
    kind === "folder" ? folderAndDescendantIds(item.id) : new Set();
  const destinations = state.playlistFolders.filter(
    (f) => !excludedIds.has(f.id),
  );

  const rows = [{ id: "", label: tr("playlistFolders.rootLevel") }].concat(
    destinations.map((f) => ({ id: f.id, label: playlistFolderPath(f) })),
  );

  const bodyHTML =
    "<div class='add-music-list' id='moveItemList'>" +
    rows
      .map(
        (r) => `
  ...
  $("moveItemList")
    .querySelectorAll(".add-music-row")
    .forEach((row) => {
...
async function exportPlaylistAsM3U(playlist) {
  ...
  withPaths.forEach((t) => {
    ...
    lines.push(
      `#EXTINF:${secs},${t.artist || "Unknown Artist"} - ${t.title || "Unknown Title"}`,
    );
  ...
  if (result && result.saved) {
    alert(
      skipped > 0
        ? tr("playlist.exportedWithSkipped", { count: skipped })
        : tr("playlist.exported"),
    );
...
function addToPlaylist(playlistId, trackId) {
  ...
  if (
    state.filter &&
    state.filter.playlistId === playlistId &&
    !state.filter.tracks.some((t) => t.id === trackId)
  ) {
...
function openAddToPlaylistModal(track) {
  ...
  $("addToPlaylistList")
    .querySelectorAll(".add-music-row[data-playlist-id]")
    .forEach((row) => {
...
function openAddMusicModal(playlistId) {
  ...
  if (!allTracks.length) {
    openModal(
      tr("modal.addMusic"),
      `<p class='info-empty'>${escapeHTML(tr("empty.noLibraryForAddMusic"))}</p>`,
    );
    return;
  }
  ...
  function renderRows() {
    ...
    const visible = sortAddMusicTracks(
      q ? allTracks.filter((t) => matchQuery(t, q)) : allTracks,
      sortSelect.value,
    );
...
function toggleFavorite(track) {
  ...
  if (!fav) return;
  let liked;
  if (fav.trackIds.includes(track.id)) {
    fav.trackIds = fav.trackIds.filter((id) => id !== track.id);
    liked = false;
  } else {
    fav.trackIds.push(track.id);
    liked = true;
  }
...
function animateLoveIcon(liked) {
  ...
  if (icon)
    replayMotion(icon, liked ? "heart-rise" : "heart-sink", liked ? 640 : 620);
...
function spawnHeartSparks(btn) {
  ...
  glyphs.forEach((g) => {
    ...
    spark.style.setProperty(
      "--rot",
      Math.round(Math.random() * 50 - 25) + "deg",
    );
...
function notifyTracksDeleted(tracks) {
  ...
  if (paths.length)
    document.dispatchEvent(
      new CustomEvent("playnck:tracks-deleted", { detail: { paths } }),
    );
}
...
function deleteTrack(track) {
  ...
  if (state.filter)
    state.filter.tracks = state.filter.tracks.filter((t) => t.id !== track.id);
  renderTab();
}
...
function getSelectedTrackIds() {
  ...
  if (state.selectType === "folders") {
    return state.tracks
      .filter((t) => ids.includes(t.folderId))
      .map((t) => t.id);
  }
  return [];
}
...
function openAddSelectedToPlaylistModal() {
  ...
  if (!state.playlists.length) {
    openModal(
      tr("sel.addToPlaylist"),
      `<p class='info-empty'>${escapeHTML(tr("empty.noPlaylistsForAdd"))}</p>`,
    );
    return;
  }

  const bodyHTML =
    "<div class='add-music-list' id='selPlaylistList'>" +
    state.playlists
      .map((p) => {
