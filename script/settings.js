...
function savePlayerBg() {
  idbPut("settings", { key: "playerBg", value: state.playerBg });
}
...
function updateSectionView() {
  ...
  switch (info.state) {
    case "checking":
      return {
        dot: "checking",
        text: tr("updates.checking"),
        btn: tr("updates.checkingBtn"),
        disabled: true,
        action: null,
      };
    case "available":
      return {
        dot: "available",
        text: tr("updates.foundDownloading", { version: info.version || "?" }),
        btn: tr("updates.downloadBtn"),
        disabled: false,
        action: "download",
      };
    case "downloading":
      return {
        ...
        text:
          ...
          (info.percent != null ? " " + info.percent + "%" : ""),
        btn: tr("updates.downloadingBtn"),
        disabled: true,
        action: null,
      };
    case "downloaded":
      return {
        dot: "downloaded",
        text: tr("updates.readyRestart", { version: info.version || "?" }),
        btn: tr("updates.restartInstall"),
        disabled: false,
        action: "install",
      };
    case "up-to-date":
      return {
        dot: "up-to-date",
        text: tr("updates.upToDate") + (version ? " (" + version + ")" : ""),
        btn: tr("updates.checkForUpdates"),
        disabled: false,
        action: "check",
      };
    case "error":
      return {
        dot: "error",
        text: info.message || tr("updates.couldntCheck"),
        btn: tr("updates.tryAgain"),
        disabled: false,
        action: "check",
      };
    default:
      return {
        dot: "idle",
        text: version ? tr("updates.running", { version }) : "",
        btn: tr("updates.checkForUpdates"),
        disabled: false,
        action: "check",
      };
...
async function onUpdateActionClick() {
  ...
  if (result && result.started === false) {
    state.updateInfo = {
      state: "error",
      message: result.reason || tr("updates.couldntCheck"),
    };
    refreshUpdateUI();
  }
}
...
function openSettingsModal() {
  ...
  $("playerBgChooseBtn").addEventListener("click", () =>
    playerBgFileInput.click(),
  );
  ...
  $("eqPresetRow")
    ...
    .forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        saveEqSettings();
        $("eqBandsRow")
          .querySelectorAll(".eq-band-slider")
          .forEach((slider, i) => {
            slider.value = state.eq.gains[i];
            const valEl = $("eqBandValue" + i);
            if (valEl)
              valEl.textContent =
                (state.eq.gains[i] > 0 ? "+" : "") + state.eq.gains[i];
          });
      });
    });
  $("eqBandsRow")
  ...
  $("gaplessEnabledToggle").addEventListener("change", (e) => {
    ...
    idbPut("settings", {
      key: "gapless",
      value: { enabled: state.gapless.enabled },
    }).catch(() => {});
  });
  ...
  $("visualizerEnabledToggle").addEventListener("change", (e) => {
    ...
    if (opacityRow)
      opacityRow.classList.toggle("disabled", !state.visualizer.enabled);
  });
  ...
  visualizerOpacitySlider.addEventListener("input", () => {
    state.visualizer.intensity = Math.max(
      0,
      Math.min(2, Number(visualizerOpacitySlider.value) || 0),
    );
    $("visualizerOpacityValue").textContent =
      Math.round(state.visualizer.intensity * 100) + "%";
    saveVisualizerSettings();
  });
