...
const state={
  ...
  currentTab:"songs",      // which sidebar tab is active: home|songs|albums|artists|playlists|folders
...
function wireDragAndDropPlay(){
  ...
  window.addEventListener("dragover",(e)=>{
    if(!isFileDrag(e)) return;
    e.preventDefault(); // required — without this the browser/Electron just navigates to/opens the file itself instead of firing "drop"
  });
...
function renderTab(){
  ...
  searchToggle.classList.toggle("hidden", !state.filter && state.currentTab==="home");
  if(!state.filter && state.currentTab==="home"){ searchRow.classList.add("hidden"); searchInput.value=""; }
