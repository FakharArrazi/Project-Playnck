...
ipcMain.handle(
  ...
  (event, backgroundColor, symbolColor) => {
    if (!mainWindow || !isHexColor(backgroundColor) || !isHexColor(symbolColor))
      return false;
  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      fs.readFile(filePath, (err, data) => {
        ...
        res.writeHead(200, {
          "Content-Type": STATIC_MIME_MAP[ext] || "application/octet-stream",
        });
    ...
    server.once("error", (err) => {
      ...
      console.warn(
        `Port ${PREFERRED_PORT} is already in use — falling back to a random port. Your saved library/theme from previous sessions won't show up until port ${PREFERRED_PORT} is free again.`,
      );
...
function findAudioArg(argv) {
  return argv.find((arg) =>
    AUDIO_EXTS.includes(path.extname(arg).toLowerCase()),
  );
}
...
ipcMain.handle("check-for-updates", async () => {
  if (!app.isPackaged) {
    return {
      started: false,
      reason: "Updates only run in the installed app, not in development.",
    };
  }
  try {
...
ipcMain.handle("trash-file", async (event, filePath) => {
  if (!filePath)
    return { trashed: false, reason: "No file path known for this track." };
  try {
...
ipcMain.handle("rename-file", async (event, filePath, desiredBaseName) => {
  if (!filePath)
    return { renamed: false, reason: "No file path known for this track." };

  const safeBase = String(desiredBaseName || "")
    .replace(/[\\/:*?"<>|]/g, "-")
    .trim();
  if (!safeBase)
    return { renamed: false, reason: "That name isn't valid for a file." };
...
ipcMain.handle("ffmpeg-install", async () => {
  try {
    return await ffmpegBridge.installFFmpeg((line) => {
      if (mainWindow)
        mainWindow.webContents.send("ffmpeg-install-progress", { line });
    });
  } catch (err) {
...
ipcMain.handle(
  "convert-resolve-output-path",
  async (event, outputDir, baseName, ext, mode) => {
    try {
      return await ffmpegBridge.resolveOutputPath(
        outputDir,
        baseName,
        ext,
        mode,
...
ipcMain.handle(
  "save-text-file",
  async (event, defaultName, content, filterName, filterExts) => {
    try {
      const result = await dialog.showSaveDialog(mainWindow, {
        defaultPath: defaultName,
        filters: [
          {
            name: filterName || "Text file",
            extensions: filterExts && filterExts.length ? filterExts : ["txt"],
          },
        ],
      });
