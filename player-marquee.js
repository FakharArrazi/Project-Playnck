(function () {
  ...
  const LINE_SELECTOR =
    "#playerPanel .track-meta .t-title, #playerPanel .track-meta .t-artist, #playerPanel .track-meta .t-album";
  ...
  function init() {
    ...
    if (reduceMotionQuery.addEventListener)
      reduceMotionQuery.addEventListener("change", refreshAll);
  }
  ...
  function handleTextMutations(mutationList) {
    ...
    mutationList.forEach((m) => {
      ...
      const line =
        node && node.closest
          ? node.closest(".t-title, .t-artist, .t-album")
          : null;
